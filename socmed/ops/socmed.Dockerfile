...
RUN apt-get update && \
	...
	rm -rf /var/lib/apt/lists/*
COPY socmed/ops/supervisor.conf /etc/supervisor/supervisord.conf
RUN mkdir /run/php && mkdir -p /var/www/apps

COPY . /var/www/apps/
COPY socmed/ops/www.conf /etc/php/7.2/fpm/pool.d/
COPY socmed/ops/php7proxy /etc/nginx/php7proxy
COPY socmed/ops/nginx.conf /etc/nginx/
COPY socmed/ops/apps.conf /etc/nginx/conf.d/apps.conf
COPY socmed/ops/php.ini /etc/php/7.2/fpm
